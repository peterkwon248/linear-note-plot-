...
## 2026-04-14 저녁 후속 — 문서 전수 정비 완료 (PR #195)

사용자 요청: "위키 관련 기존 메모한 내용들 최신화. 삭제할 건 삭제, 정밀화할 건 정밀화"

### 실행 결과 (Phase 0)
- 13개 문서 검토 → 9개 업데이트 (+ 2개 신규)
- 진실의 원천 단일화: `docs/BRAINSTORM-2026-04-14-column-template-system.md`
- 폐기된 문서들에 DEPRECATED 배너 + 새 문서 링크 (히스토리 보존)
- 모든 상호 참조 재검증

### PR #195 merged
- session: 인포박스 편집 엉킴 수정 + 위키 템플릿 시스템 재설계
- merge commit: 9eecca15
- 다음 세션에서 Phase 1 시작 가능 상태

## 2026-04-21 후반 — Book 에디터 UX 대정돈 (다수 결정)

**Wiki PR 산출물 전면 재사용 (사용자 규칙)**
- **결정**: Book shells의 에디터/picker/renderer는 자체 구현 대신 기존 Wiki PR의 export된 컴포넌트 재사용
- 근거: 사용자 피드백 "이전 PR들 참고하면 되지 않나??" (2회, 에디터 허접 + Add Block 둘 다)
- 적용: `BookInlineEditor` → `WikiTextEditor`, Book 자체 8-option picker → `AddBlockButton` (13 옵션), Book별 렌더 → `WikiBlockRenderer` fallback
- 영구 규칙: 새 기능 구현 전 **먼저 `Grep "^export function"`로 기존 코드 검색**

**Phase 3A 드래그 reorder 범위 제한**
- 이번 세션: Wiki shell만 `BookDndProvider` 적용
- Magazine/Newspaper/Book/Blank는 Phase 3A-2로 분리
- 근거: Magazine(`columnCount` CSS) + Book(dropcap float)의 dnd-kit `transform` 충돌 가능성 별도 검증 필요

**Display 팝오버 Book 전용 섹션**
- Notes의 `DisplayPanel` 구조 (List/Board, Grouping, Properties) 억지로 맞추지 않음
- Book 전용 섹션 10개: Shell / Render mode / Typography / Margins / Columns / Chapter breaks / Background / Card border / Ink colors / Decoration
- 내부적으로 Section/Chip/ColorRow/CheckRow 헬퍼로 구조화

**Grid Editor mode 완전 폐기 (코드 삭제)**
- book-editor.tsx의 `mode === "editor"` 브랜치 + GridEditor import 전부 삭제
- 근거: BRAINSTORM-2026-04-21-book-ux-refinement.md "Grid Editor 폐기, 인라인 드래그로 대체"

**TweakPanel → BookDisplayPanel로 이관 + 삭제**
- 기존 fixed sidecar 패널 제거, ViewHeader의 Display 팝오버로 흡수
- `components/book/tweak-panel.tsx` 삭제
- 근거: 상단 ViewHeader Display 버튼이 Notes 패턴 표준

**WikiTextEditor의 per-block FixedToolbar 유지**
- 대안 "글로벌 하단 FixedToolbar 하나"는 고려 안 함
- 근거: wiki/notes와 동일 패턴 유지 (일관성 > 시각적 clean)
