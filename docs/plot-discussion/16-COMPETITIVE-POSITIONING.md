...
1. **Note/Wiki 통합** -> "뭐든 만들 수 있는"의 첫걸음
2. **Home 공간** -> "워크스페이스" 느낌의 진입점
3. **Pages 네이밍** -> "노트앱" 꼬리표 떼기
4. **인라인 쿼리 뷰** -> "문서 안에서 데이터 보기" (Coda/노션 인라인 DB 방향)
5. **투두 시스템** -> "워크스페이스"의 기본 기능
6. **캔버스 뷰** -> AFFiNE 참고, 문서<->캔버스 동기화 (P3~P4)
