...
> **Status**: Draft v1.2 (2026-05-07) — Phase 0/1 완료, **Phase 2 DEFERRED**
...
## 🔴 DECISION (2026-05-07): Phase 2 (Imperial Icon Kit) DEFERRED

**결정**: Imperial icon kit 전면 도입 **보류**. 기존 phosphor-icons 그대로 유지.

**근거**:
- 직전 인벤토리 (`.omc/plans/v3-phosphor-inventory.md`)가 **부정확** (4 icons / 2 files 주장 → 실측 **60+ icons / 119 files** phosphor SSR 사용)
- 119 files = 단일 PR로 안전하지 않음 (작업 원칙 #2 "최소 diff" 위배)
- 시각 위화감 미미: phosphor `weight="regular"` (1.5–1.7px stroke) ≈ Imperial (1.5px stroke). Linear-style 동일 → Imperial 도입의 시각 가치 약함
- 현 빌드 정상: `tsc --noEmit` 0 errors / `npm run build` clean / 185 tests pass
- phosphor + lucide 등 외부 라이브러리 추가는 의미 없음 (이미 phosphor 광범위)

**현 상태 유지** (revert 안 함):
- `components/icons/imperial.tsx` + `imperial-extras.tsx` 모듈 보존 (activity-bar 등 일부 사용 중)
- Phase 2 partial work (activity-bar / plot-icons `IconWiki` / `note-split`·`wiki-merge`·`wiki-split`-page 일부 / `backlink-card` weight 제거) 그대로 유지
- 신규 코드는 기존 패턴 (phosphor SSR import) 사용 OK — Imperial과 섞여도 시각 위화감 거의 0
...
**아래 §5.3, §6.2 등 Phase 2 본문은 historical reference로 보존**. §0 TL;DR / §3 Success Metrics G3 / §6.2 Phase 2 acceptance criteria는 보류 상태.
...
- ~~**Imperial icon kit**: phosphor-icons (실측 **2 files / 4 icons** — 인벤토리 결과 `.omc/plans/v3-phosphor-inventory.md` 참조) + 자체 SVG (`components/plot-icons.tsx` 22+ icons) + lucide/iconoir/tabler/remixicon 혼재 → Imperial (1.5px stroke, 24 viewBox, currentColor)~~ — **DEFERRED 2026-05-07** (인벤토리 부정확, 실측 60+ icons / 119 files. 위 DECISION 섹션 참조)
