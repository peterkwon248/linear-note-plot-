# Plot — 다중 기기 Sync 설계 결정 (2026-04-29 보류, 다음 세션 결정)

> 큰 방향 전환. 영구 규칙 "큰 방향 전환 전 전체 설계 확정" 적용.
> 결정 6개 받기 전 어떤 코드도 시작 X.
> 이 문서는 다음 세션 시작 시 사용자와 함께 결정할 항목 정리.
...
영구 규칙: "큰 방향 전환 전 전체 설계 확정". 6개 항목 결정 받기 전 어떤 코드도 시작 X.

다음 세션 시작 시:
1. /before-work 실행
2. NEXT-ACTION.md 읽음 → 이 문서로 안내
3. 사용자와 결정 6개 받기
4. 결정 후 → PRD 작성 (별도 문서) → phase 분할 → 구현 시작
