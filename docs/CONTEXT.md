...
### 핵심 결정 (영구)
- **앱 i18n 실태**: 코어(노트뷰/필터/위키 일부)만 됐고 **Books·사이드패널·다이얼로그·에디터 툴바는 통째 영어**였음 = whack-a-mole 아니라 절반 미번역. 적발식→전수 스캔→고가시성 우선.
- **i18n 패턴(영구)**: 병렬 executor wiring(하위 컴포넌트마다 `const t=useT()`)+키맵 반환 → KO 일괄(용어 일관) → `git diff`로 EN 복원 → 참조키 grep 전수검증. 데이터 라벨(블록/프리셋)=`t(\`...${id}...\`)` 렌더 분기(데이터 보존). ProseMirror/TipTap config=`translate()`.
- **🔴 빌드 검증 함정(영구)**: `npm run build 2>&1 | tail` 금지(tail이 npm 실패 exit code 가림) + incremental TS 캐시 false-clean → **`rm -rf .next && npm run build` 파이프 없이, "Failed to compile" 직접 확인**. tsc는 미존재 i18n 키 못 잡음(grep 검증 필수).
- **거터 affordance=호버 노출**(opacity-0 at rest), 실험 배지=dev 전용.

### 완료 (PR #538, 6커밋)
- 필터 드롭다운 잘림 / placeholder 동작(업노트, "삭제 안 됨" 신고의 정체) / v139 위키템플릿 부활 버그 / i18n ~500+(위키·에디터·Books·패널·다이얼로그·사이드바·헤더툴팁·블록라벨) / 에디터 버그2(북마크 거터 opacity·yjs 배지 dev게이팅).

### 다음 (P0)
1. 남은 영어 i18n 2차(~150): 설정 옵션·온톨로지 그래프·뷰 leftover(library/templates/folder-detail/inbox/home)·인포박스 프리셋 라벨(데이터 레이어).

### Store version
무변경(v154). migrate 로직 정정(스키마 불변).
...
### 핵심 결정 (영구)
- **랜딩 = 실제 코드 기반**(에이전트 3개 파일:라인 검증, 지어낸 것 0). 미구현 주장 금지(협업/싱크/모바일/다국어 X). `lib/colors.ts` 실제 hex + Pretendard.
- **노트→위키 "승격"** = promote 넛지(제안)+위키 import(note-ref 임베드)+assembly. 옛 convertToWiki 데드 제거. 카피 "노트가 위키로 자랍니다"(변신 오해 회피).
- **헤드리스 Chrome(`--headless=old --screenshot`) = 이 환경 유일 신뢰 스샷**(preview MCP 타임아웃). 앱 기본 로케일 en.

### 완료
- 랜딩(`site/index.html` 9섹션 + 실제 스샷 5장 + EN/KO 토글 KO기본) / 북 디테일바(`books-view handleSelect` peek 동기화) / 온톨로지 한글화(i18n +25키 + 2컴포넌트 useT).

### 다음 (P0)
1. 랜딩 섹션별 사용법+스샷 보강(0.001, 사용자 지정). 2. (carry) 타임라인 #10/#11(0.005).

### Store version
무변경(v154). 신규 `site/`=정적(빌드 불필요).
...
### 핵심 결정 (영구)
- **무서명 v0.1.0 합의**: Azure($9.99/월) 한국 자격 미달 공산, OV/EV 유저 0 과투자. 무서명=정상+SmartScreen 1회 클릭(macOS는 하드블록, Apple $99+공증 필수=Windows 한정 관용). MS Store MSIX=우회 후속.
- **config**: publisher Plot / identifier com.plot.desktop(.app 제거) / NSIS installMode currentUser(per-user 무admin).

### 완료
- release 빌드(콜드 3.5분) + `.msi`(7.7MB)+`.exe`(6.6MB) + plot.exe 스모크(WebView2). NSIS flaky(`os error 5` 부분추출) 캐시 삭제 해결. Manufacturer Plot 검증. 앱/Store 무변경(v154).

### 다음 (P0)
1. P3 export/백업 점검 + 출시 전 안정화. 2. (별개) 폴더 필터 F5 / macOS 서명 / MS Store.
...
### 핵심 결정 (영구)
- 삭제 정확성=상용화 핵심(부활0·orphan0·완전삭제). **별도 IDB store**(노트본문/위키blocks/mention/첨부) 영구삭제 시 같이 지워야 — **위키 blocks 누락 확정**(`deleteArticleBlocks` 미호출).
- 시드=1회성이어야(re-seed "비면 부활" 버그, `index.ts:319`). OS휴지통은 P2(.md 파일)서 가능.

### 다음 우선순위 (P0)
1. 데이터 라이프사이클 전수 감사+수정(출시 전 필수). 2. 데스크톱 캐치올 라우팅. 3. carry.
...
### 핵심 결정 (영구)
- **레이아웃도 리니어 완벽 모방**: 3-zone 골격 유지 + 그 안 디테일(모션/호버/색/보조액션 배치)을 리니어 문법으로. **MIRROR**(리니어 푼 것)+**EXTRAPOLATE**(우리 고유 zone). 핵심 미덕=**절제**(안 보여줄 건 ⌘K/풀페이지로 숨김). Track B(셸 리디자인) 착수.
- **폰트 Pretendard**(Inter 메트릭 복제+한글 네이티브, 다국어). **수직 슬라이스 전략**(넓게=체감0 → 한 표면 깊게 → 토큰 확립 → 전파).
- **리니어 검색 = 3-way 공존**(⌘K 팔레트 / `/` 전역검색 / ⌘F 뷰내. 양자택일 X). **"뒤죽박죽"=미완성 리팩터**(셸 크롬 이사 절반 → ⌘K 팔레트 고아+죽은코드+과노출). **Trash**=리니어는 계정메뉴에 안 둠 → 사이드바 강등.
- **A3.2 스키마 엔진**: `PropertyDef[]`(6-카테고리)→어댑터→4표면 자동생성 = config 역전. 소비 0수정. 진짜 동등성=원본 스냅샷 비교(swap 후 테스트=tautology).

### 완료
- 폰트 Pretendard / A3.2 스키마 엔진(PR #495) / A3.3 필터 크롬(divider/16px/칩바/-965줄) / 노트행 모션(oklch 토큰+zero-shift) / 셸 1차(⌘K 팔레트 복구/죽은코드-127/avatar 드롭다운/userName 이니셜).

### 기술 학습
- 이 환경 preview eval = route 전환+키 이벤트(⌘K) dispatch가 module state라 불가 → visible 검증=사용자 실화면.
- 모션 zero-shift = opacity/visibility(공간 미리 확보), display/width 변동 X.
