  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align={align} className="w-[280px] p-3">
        ...
        <div className="flex flex-col gap-0.5">
          {presets.map(({ key, label, formatted, iso }) => (
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 text-note rounded-md hover:bg-hover-bg transition-colors"
            >
              <span className="text-foreground font-medium">{label}</span>
              <span className="text-muted-foreground text-2xs">{formatted}</span>
            </button>
          ))}
        </div>
        ...
        {/* Custom date section */}
        <div>
          <p className="text-2xs font-medium text-muted-foreground px-3 mb-2">
            Custom
          </p>
          <Calendar
            mode="single"
          ...
          <div className="flex items-center gap-2 mt-2 px-1">
            <input
              ...
              className="h-8 w-full rounded-md border border-border bg-background px-2 text-note focus:outline-none focus:ring-1 focus:ring-ring"
            />
            <button
              type="button"
              onClick={handleSetCustom}
              disabled={!selectedDate}
              className="bg-accent text-accent-foreground text-note font-medium rounded-md px-3 py-1.5 hover:bg-accent/80 disabled:opacity-50 whitespace-nowrap transition-colors"
            >
              Set
            </button>
          </div>
