    ...
    <div className="w-[260px] shrink-0 border-l border-border/50 overflow-y-auto px-4 py-5">
      ...
      <Section title="Categories">
        {categories.items.length === 0 && categories.uncategorized === 0 ? (
          <p className="text-xs text-muted-foreground/40">No categories</p>
        ) : (
          <div className="space-y-0.5">
            ...
            {categories.uncategorized > 0 && (
              <span className="inline-block rounded-[5px] bg-chart-3/5 px-1.5 py-0.5 text-xs font-medium text-chart-3/60">
                Uncategorized
      ...
      <Section title="Recent">
        {recentChanges.length === 0 ? (
          <p className="text-xs text-muted-foreground/40">No recent changes</p>
        ) : (
          ...
          <div className="space-y-px">
            {redLinks.slice(0, 5).map((item) => (
              <div key={item.title} className="group flex items-center gap-2 rounded-md px-2 py-[7px] transition-colors duration-100 hover:bg-hover-bg">
                ...
                <span className="shrink-0 text-xs tabular-nums text-muted-foreground/30 group-hover:hidden">
                  {item.refCount}
                </span>
                <button
                  onClick={() => onCreateFromRedLink(item.title)}
                  className="hidden shrink-0 items-center gap-0.5 text-xs font-medium text-accent group-hover:flex"
                >
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h4 className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground/40">
        {title}
      </h4>
      {children}
    </div>
