 ...
 * surfaces; the master order = the FILTER category order, and:
 *   - sort order   falls out of declaration order of `isSortable` props
 *     (updatedAt, createdAt, title, links, reads),
 *   - group order  = `isGroupable` props (status, folder, label, parent) then
 *     the group-only `extraGroupings` (role, family, date, firstLetter),
 *   - display order diverges (label/tags before folder) and is reproduced via
 *     explicit `displayOrder` hints.
 *
...
const PROPERTIES: PropertyDef[] = [
  /* 1. status — filter / display / group (not sortable). */
  {
    key: "status",
    category: "workflow",
    label: "Status",
  ...
  /* 2. priority — board-only display badge. */
  {
    key: "priority",
    category: "workflow",
    label: "Priority",
  ...
  /* 4. label — filter / board-display / group. */
  {
    key: "label",
    category: "classification",
    label: "Label",
  ...
  /* 7. parent — display / group (family-distinct relation). */
  {
    key: "parent",
    category: "relations",
    label: "Parent",
  ...
  /* 8. updatedAt — filter ("Dates") / sort ("Updated") / display ("Updated").
  ...
  /* 10. createdAt — sort ("Created") / display ("Created"). */
  {
    key: "createdAt",
    category: "time",
    label: "Created",
  ...
  /* 12. links — filter ("Links" buckets) / sort ("Links") / display ("Backlinks"). */
  {
    key: "links",
    category: "relations",
    label: "Links",
  ...
  /* 13. wikiRegistered — filter only (note ↔ wiki membership). */
  {
    key: "wikiRegistered",
    category: "relations",
    label: "Wiki",
  ...
  /* 14. content — filter only (has image / code / table). */
  {
    key: "content",
    category: "content",
    label: "Content",
  ...
  /* 16. wordCount — display column "Words"; sorts via SortField "reads"
