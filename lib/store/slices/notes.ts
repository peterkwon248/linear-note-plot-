  ...
  return {
    createNote: (partial?: Partial<Note>) => {
      ...
      const newNote: Note = {
        ...
        ...workflowDefaults(partial?.status ?? "backlog"),
    ...
    splitNote: (sourceId: string, { extractedJson, remainingJson, newTitle }: { extractedJson: Record<string, unknown>; remainingJson: Record<string, unknown>; newTitle: string }): string | null => {
      ...
      // Status defaults to "backlog" — a freshly split-off note belongs in triage.
      const newNote: Note = {
        id: newId,
        title: titleForNew,
        content: "",
        ...
        notes: state.notes.map((n: Note) =>
          n.id === id
            ? {
                ...
                // returns to its original backlog/todo/in_progress/done bucket
    ...
    convertToWiki: (noteId: string) => {
      set((state: any) => ({
        notes: state.notes.map((n: Note) =>
          n.id === noteId
            ? { ...n, noteType: "wiki" as const, updatedAt: now(), lastTouchedAt: now() }
            : n
        ),
      }))
    ...
    revertFromWiki: (noteId: string) => {
      set((state: any) => ({
        notes: state.notes.map((n: Note) =>
          n.id === noteId
            ? { ...n, noteType: "note" as const, updatedAt: now(), lastTouchedAt: now() }
            : n
        ),
      }))
